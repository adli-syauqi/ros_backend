...
class MapDrive(Node): 
	def __init__(self):
		...
		self.sub_executor.add_node(self.sub_node)
		...
		self.declare_parameter("output_filename", "~/pose_output.yaml")
		self.filename = os.path.expanduser(self.get_parameter("output_filename").get_parameter_value().string_value)
	...
	def do_boustrophedon(self, poly, costmap):
		...
		for ix in range(int(minx), int(maxx)):
			...
			for iy in range(int(miny), int(maxy)):
				...
				if not poly.contains(Point([x,y])):
					# Outside polygon: treat as obstacle
					column.append(0)
				elif data == -1:
					# Unknown: treat as freespace, matches Nav2 allow_unknown=true
					column.append(-1)
		...
		#	print("9: ") # -------------------------------------
			...
			try:
				result = subprocess.run(["ruby", boustrophedon_script, ftmp.name], capture_output=True, text=True); self.get_logger().info("RUBY STDOUT: " + result.stdout[:300] + " STDERR: " + result.stderr[:300])
	...
	def _getPathImpl(self, start, goal, planner_id='', use_start=False):
		...
		rclpy.spin_until_future_complete(self.sub_node, send_goal_future, executor=self.sub_executor)
		...
		rclpy.spin_until_future_complete(self.sub_node, self.result_future, executor=self.sub_executor)
	...
	def private_shutdown(self):
		...
		if self.result_future:
			...
			rclpy.spin_until_future_complete(self.sub_node, future, executor=self.sub_executor)
